...
auto create_he3_cut = [](int telescope_number) {
    ...
    for (ushort i = 0; i < 16; i++) {
        ...
        channel_cut.Form("(a20_ch == %d && cuthe3_%d_er_%d)", er_channel, telescope_number, i);
        cut += channel_cut;
        if (i != 15)
            cut += " || ";
    }
...
auto create_he4_cut = [](int telescope_number) {
    std::map<ushort, ushort> inverse16Mapping = {
        {0, 15},
        {1, 14},
        {2, 13},
    ...
    for (ushort i = 0; i < 16; i++) {
        ...
        channel_cut.Form("(a20_ch == %d && cuthe4_%d_er_%d)", er_channel, telescope_number, i);
        cut += channel_cut;
        if (i != 15)
            cut += " || ";
    }
